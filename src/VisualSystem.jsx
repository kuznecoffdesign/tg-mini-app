...
export function WireIllustration({variant=0}) {
  return <svg className={`wire-illustration wire-${variant}`} viewBox="0 0 360 270" fill="none" aria-hidden="true">
    {variant===0 ? [4,3,2,1,0].map((n)=><g key={n} className="wire-layer" style={{animationDelay:`${(4-n)*65}ms`}} transform={`translate(0 ${n*22})`}><path className="wire-surface" d="M76 81 180 27 284 81 180 137Z"/><path d="M76 81v16l104 56 104-56V81M76 81l104 56 104-56M180 137v16"/><path d="m76 81 104-54 104 54"/>{n===0&&<path className="wire-blue wire-check" d="m151 72 20 11 38-21m-58 31 20 11 38-21"/>}</g>) : variant===1 ? <><path className="wire-connector" d="M95 102 180 148 265 102M180 148v65"/>{[[45,35],[215,35],[130,145]].map(([x,y],i)=><g key={x} className="wire-layer" style={{animationDelay:`${i*65}ms`}} transform={`translate(${x} ${y})`}><path className="wire-surface" d="m0 30 50-26 50 26v57l-50 27L0 87Z"/><path d="m0 30 50 27 50-27M50 57v57"/><path className={i===2?'wire-blue':''} d="m38 27 12-6 12 6-12 6Z"/></g>)}</> : [5,4,3,2,1,0].map(n=><g key={n} className="wire-layer" style={{animationDelay:`${(5-n)*65}ms`}} transform={`translate(${n*19} ${-n*10})`}><path className="wire-surface" d="M76 112q0-6 6-3l116 62q6 3 6 9v75q0 6-6 3L82 196q-6-3-6-9Z"/><path d="m90 136 88 47m-88-32 62 33" className={n===0?'wire-blue':''}/></g>)}
  </svg>;
}

export function NewHero({cta,caseLink}) {return <section className="new-hero container"><Reveal className="hero-intro"><h1 className="hero-title"><span className="hero-title-line"><span>Создаём продукты</span></span><span className="hero-title-line"><span>внутри Telegram<i>.</i></span></span></h1><div className="new-hero-summary"><p>Telegram Mini Apps и боты для магазинов, сервисов<br className="desktop-break"/> и внутренних продуктов. От сценария до работающей системы.</p><div className="new-hero-actions">{cta}{caseLink}</div></div></Reveal><Reveal className="hero-workspace"><WorkspacePanel/></Reveal></section>;}
